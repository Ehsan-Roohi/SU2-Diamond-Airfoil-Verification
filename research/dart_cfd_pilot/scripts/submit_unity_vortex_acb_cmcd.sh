...
#SBATCH --output=logs/vortex-acb-%j.out
#SBATCH --error=logs/vortex-acb-%j.err
...
readonly MFC_ROOT="${VORTEX_ACB_MFC_ROOT:-/project/pi_roohie_umass_edu/github_sync/KineticGaussian/SU2-Diamond-Airfoil-Verification/third_party/MFC-0c9a1d43-iles-portable-v3}"
readonly CASE_DIR="${VORTEX_ACB_MFC_CASE_DIR:-${WORK_ROOT}/ccfcv-alpha30-raw}"
readonly CCFCV_DIR="${VORTEX_ACB_CCFCV_DIR:-${PROJECT_ROOT}/research/dart_cfd_pilot/results/63828431}"
readonly TEST_PYTHON="${WORK_ROOT}/conda/dart-sam3-py311/bin/python"
readonly MFC_PYTHON="${MFC_ROOT}/build/venv/bin/python3"
readonly RUN_ID="${SLURM_JOB_ID:-acb-cmcd-manual}"
readonly OUTPUT_DIR="${PROJECT_ROOT}/research/dart_cfd_pilot/results/${RUN_ID}"
readonly ARCHIVE="${PROJECT_ROOT}/VORTEX_ACB_CMCD_${RUN_ID}_COMPLETE.tar.gz"
...
grep -q '"claim_gate": "frozen_cmcd_transfers_to_alpha30_expert_labels_and_third_case_next"' \
    "${CCFCV_DIR}/ccfcv_report.json" || { echo "ERROR: CC-FCV prerequisite gate did not pass" >&2; exit 6; }
...
"${TEST_PYTHON}" -m pytest -q \
    research/dart_cfd_pilot/tests/test_vortex_acb_cmcd.py
PYTHONPATH="${MFC_ROOT}/toolchain${PYTHONPATH:+:${PYTHONPATH}}" \
    ...
    --ccfcv-dir "${CCFCV_DIR}" --output-dir "${OUTPUT_DIR}"
...
} > "${OUTPUT_DIR}/acb_cmcd_environment.txt"

tar --no-same-owner -C "${OUTPUT_DIR}" -czf "${ARCHIVE}" .
sha256sum "${ARCHIVE}" > "${CHECKSUM}"
echo "ACB_CMCD_RC=0"
echo "ACB_CMCD_OUTPUT_DIR=${OUTPUT_DIR}"
echo "ACB_CMCD_ARCHIVE=${ARCHIVE}"
echo "ACB_CMCD_CHECKSUM=${CHECKSUM}"
