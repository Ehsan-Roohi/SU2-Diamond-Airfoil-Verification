#!/usr/bin/env python3
"""Restart the MFC Mach-3/AoA-40 startup case and continue it to t=3."""
...
parser.add_argument("--start-step", type=int, default=4320)
parser.add_argument("--stop-step", type=int, default=16200)
parser.add_argument("--save-every", type=int, default=108)
args = parser.parse_args()

if args.start_step <= 0:
    parser.error("--start-step must be positive")
...
if args.start_step % args.save_every or args.stop_step % args.save_every:
    parser.error("--start-step and --stop-step must be divisible by --save-every")
...
# This restart file intentionally omits the original domain extents and IC
# patch. MFC reads both the old grid and the state at t_step_start from the
# existing restart_data/ directory beside this file. t_step_old is the start
# of the original run, as required by MFC's restart interface.
m = 2969
n = 2699
x_beg, x_end = -5.0, 6.0
y_beg, y_end = -5.0, 5.0
