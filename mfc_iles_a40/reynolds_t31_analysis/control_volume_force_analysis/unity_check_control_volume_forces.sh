...
if (($#)); then
    ...
    pointer="$default_parent/LAST_TIM_COLONIUS_CONTROL_VOLUME_FORCE_OUTPUT.txt"
...
archive="$(dirname -- "$output")/TIM_COLONIUS_CONTROL_VOLUME_FORCES_JOB${job_id}.zip"
...
if [[ -f "$output/slurm-$job_id.out" ]]; then
    tail -n 60 "$output/slurm-$job_id.out"
else
...
if [[ -s "$output/ANALYSIS_COMPLETE.txt" && -s "$archive" ]]; then
    ...
    case "$status" in
        PASS) echo 'MFC_CONTROL_VOLUME_FORCE_CHECK=PASS' ;;
        QUALIFIED) echo 'MFC_CONTROL_VOLUME_FORCE_CHECK=QUALIFIED_CV_SENSITIVITY_REPORTED' ;;
        *) echo "MFC_CONTROL_VOLUME_FORCE_CHECK=INVALID_STATUS_$status"; exit 1 ;;
    esac
    printf 'EMAIL_FIGURE=%s\nPDF=%s\nSUMMARY=%s\nHISTORY=%s\nARCHIVE=%s\n' \
        "$output/TIM_COLONIUS_REYNOLDS_FORCE_TRENDS.png" \
        "$output/TIM_COLONIUS_CONTROL_VOLUME_FORCES.pdf" \
        "$output/control_volume_force_summary.csv" \
        "$output/control_volume_force_history.csv" \
        "$archive"
    exit 0
fi
...
if [[ -s "$output/ANALYSIS_FAILED.txt" ]]; then
    ...
    echo 'MFC_CONTROL_VOLUME_FORCE_CHECK=FAILED'
    exit 1
fi

echo 'MFC_CONTROL_VOLUME_FORCE_CHECK=RUNNING_OR_PENDING'
