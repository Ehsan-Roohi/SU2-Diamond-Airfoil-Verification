...
def main() -> int:
    ...
    replacement = f"re_chord = {args.re_chord:.17g}  # Tim Colonius high-viscosity diagnostic"
    args.output.write_text(text.replace(SOURCE_LINE, replacement), encoding="utf-8")
    print(f"HIGH_VISCOSITY_CASE=PASS Re_c={args.re_chord:.12g} output={args.output}")
    return 0
