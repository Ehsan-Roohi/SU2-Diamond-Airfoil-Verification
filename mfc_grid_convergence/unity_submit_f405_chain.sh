...
if command -v flock >/dev/null 2>&1 && [[ "${MFC_LOCK_HELD:-0}" != 1 ]]; then
    # Use flock's path form and re-exec this batch script while flock owns the
    # descriptor.  Some Unity nodes close descriptors opened by the batch
    # shell before launching flock, which made `flock ... 9` fail immediately.
